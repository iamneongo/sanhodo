  ...
  try {
    ...
    if (rows.length < 2) {
      return NextResponse.json(
        { error: "File CSV cần có dòng tiêu đề và ít nhất một món ăn." },
        { status: 400 }
      );
    }
    ...
    if (indexes.ten_mon < 0) {
      return NextResponse.json({ error: "CSV thiếu cột ten_mon / Tên món." }, { status: 400 });
    }

    const existing = await listMenuItems(context.supabase, { branchId });
    ...
    for (let rowIndex = 1; rowIndex < rows.length; rowIndex += 1) {
      ...
      const item = await createMenuItem(context.supabase, payload);
      created.push(item);
