...
export async function POST(request) {
  ...
  try {
    ...
    const objectPath = `landing-pages/${branchId || "shared"}/${Date.now()}-${crypto.randomUUID()}.${extension}`;
    ...
    if (uploadError) {
      return NextResponse.json(
        { error: uploadError.message || "Không upload được ảnh landing page lên storage." },
        { status: 500 }
      );
    }
    ...
    return NextResponse.json(
      { error: error.message || "Không upload được ảnh landing page." },
      { status: 500 }
    );
  }
}
