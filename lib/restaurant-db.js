...
import { normalizeLandingPageConfig } from "./landing-page-config";
import {
...
function getSupabaseMigrationHint(error) {
  ...
  const hints = [
    ...
    {
      matchers: ["landing_config"],
      migration: "supabase/migrations/20260515_011_branch_landing_config.sql",
      label: "schema landing page chi nhanh"
    },
    {
    ...
    {
      matchers: ["media_assets", "integration_events"],
      migration: "supabase/migrations/20260603_013_media_sync_foundation.sql",
      label: "schema media va dong bo"
    }
  ];
...
function normalizeMediaAssetRow(row) {
  return {
    id: row.id,
    branchId: row.branch_id || "",
...
function normalizeTableSessionEventRow(row) {
  return {
    id: row.id,
    branchId: row.branch_id || "",
    tableId: row.table_id || "",
...
function normalizeIntegrationEventRow(row) {
  return {
    id: row.id,
    branchId: row.branch_id || "",
    provider: row.provider || "uvfl",
...
function buildIntegrationEventValues(payload, provider = "uvfl") {
  return {
    branch_id: cleanText(payload.branchId) || null,
    provider: cleanText(provider, "uvfl") || "uvfl",
    event_type: cleanText(payload.eventType),
    source_table: cleanText(payload.sourceTable),
    source_id: cleanText(payload.sourceId) || null,
    payload: payload.payload && typeof payload.payload === "object" ? payload.payload : {},
    status: cleanText(payload.status, "pending") || "pending",
    retry_count: parseInteger(payload.retryCount, 0),
    last_error: cleanText(payload.lastError),
    synced_at: toIsoDateTime(payload.syncedAt)
  };
}
...
function inferFileNameFromUrl(value) {
  ...
  try {
    ...
    return fileUrl.split("/").filter(Boolean).pop() || "";
  }
}

function buildMediaAssetValues(payload) {
  const fileUrl = cleanText(payload.fileUrl);
  const title = cleanText(payload.title) || cleanText(payload.fileName) || inferFileNameFromUrl(fileUrl);
  return {
    branch_id: cleanText(payload.branchId) || null,
    asset_type: cleanText(payload.assetType, "image") || "image",
    category: cleanText(payload.category),
    title: title || `Media ${Date.now()}`,
    file_name: cleanText(payload.fileName) || inferFileNameFromUrl(fileUrl),
    file_url: fileUrl,
    thumbnail_url: cleanText(payload.thumbnailUrl),
    mime_type: cleanText(payload.mimeType),
    file_size: parseInteger(payload.fileSize, 0),
    width: payload.width ? parseInteger(payload.width, 0) : null,
    height: payload.height ? parseInteger(payload.height, 0) : null,
    duration_seconds: payload.durationSeconds ? parseInteger(payload.durationSeconds, 0) : null,
    status: cleanText(payload.status, "active") || "active",
...
function normalizeBranchRow(row) {
  return {
    ...
    isActive: row.is_active !== false,
    sortOrder: row.sort_order || 0,
    landingConfig: normalizeLandingPageConfig(row.landing_config || row.landingConfig || {}),
    createdAt: row.created_at || "",
    updatedAt: row.updated_at || ""
  };
}
...
function normalizeAdminLoginAuditRow(row) {
  return {
    id: row.id,
...
function buildBranchValues(payload) {
  ...
  if (payload.landingConfig) {
    values.landing_config = normalizeLandingPageConfig(payload.landingConfig);
  }
...
export async function createBranch(supabase, payload) {
  ...
  const { data, error } = await insertWithOptionalColumns(supabase, "branches", values, {
    select: true,
    single: true,
    optionalColumns: ["landing_config"]
  });
...
export async function updateBranch(supabase, id, payload) {
  validateBranchPayload(payload);
  const values = buildBranchValues(payload);
  const { data, error } = await updateWithOptionalColumns(supabase, "branches", id, values, {
    optionalColumns: ["landing_config"]
  });
...
export async function listAdminLoginAudits(supabase, options = {}) {
  const profileId = cleanText(options.profileId);
  const branchId = cleanText(options.branchId);
  const limit = Math.min(Math.max(parseInteger(options.limit, 120), 1), 500);
  let query = supabase
    .from("admin_login_audits")
    .select("*")
  ...
  if (error) {
    ...
    throw new Error(toFriendlySupabaseErrorMessage(error, "Không tải được lịch sử đăng nhập"));
  }

  return (data || []).map(normalizeAdminLoginAuditRow);
}

export async function logAdminLoginAudit(supabase, payload = {}) {
  const values = {
    profile_id: cleanText(payload.profileId) || null,
    email: cleanText(payload.email),
    role: cleanText(payload.role),
    branch_id: cleanText(payload.branchId) || null,
    login_method: cleanText(payload.loginMethod, "password") || "password",
    success: payload.success !== false,
    ip_address: cleanText(payload.ipAddress),
    user_agent: cleanText(payload.userAgent),
    failure_reason: cleanText(payload.failureReason),
    metadata: payload.metadata && typeof payload.metadata === "object" ? payload.metadata : {}
  };
  ...
  if (error) {
    ...
    throw new Error(toFriendlySupabaseErrorMessage(error, "Không ghi được lịch sử đăng nhập"));
  }

  return data ? normalizeAdminLoginAuditRow(data) : null;
}
...
export async function createDriver(supabase, payload) {
  ...
  await enqueueIntegrationEventSafe(supabase, {
    branchId: normalized.branchId,
    eventType: "DRIVER_CREATED",
    sourceTable: "drivers",
    sourceId: normalized.id,
    payload: { driver: normalized }
  });
  return normalized;
}
...
export async function updateDriver(supabase, id, payload) {
  ...
  const normalized = data ? normalizeDriverRow(data) : null;
  if (normalized) {
    await enqueueIntegrationEventSafe(supabase, {
      branchId: normalized.branchId,
...
export async function createTravelPartner(supabase, payload) {
  ...
  const normalized = normalizeTravelPartnerRow(data);
  await enqueueIntegrationEventSafe(supabase, {
    branchId: normalized.branchId,
    eventType: "TRAVEL_PARTNER_CREATED",
    sourceTable: "travel_partners",
    sourceId: normalized.id,
    payload: { partner: normalized }
  });
  return normalized;
}
...
export async function updateTravelPartner(supabase, id, payload) {
  ...
  const normalized = data ? normalizeTravelPartnerRow(data) : null;
  if (normalized) {
    await enqueueIntegrationEventSafe(supabase, {
      branchId: normalized.branchId,
      eventType: "TRAVEL_PARTNER_UPDATED",
      sourceTable: "travel_partners",
      sourceId: normalized.id,
      payload: { partner: normalized }
    });
...
export async function createPartnerBooking(supabase, payload) {
  ...
  const normalized = normalizePartnerBookingRow(data);
  await enqueueIntegrationEventSafe(supabase, {
    branchId: normalized.branchId,
    eventType: "PARTNER_BOOKING_CREATED",
    sourceTable: "partner_bookings",
    sourceId: normalized.id,
    payload: { partnerBooking: normalized }
  });
  return normalized;
}
...
export async function updatePartnerBooking(supabase, id, payload) {
  ...
  const normalized = data ? normalizePartnerBookingRow(data) : null;
  if (normalized) {
    await enqueueIntegrationEventSafe(supabase, {
      branchId: normalized.branchId,
      eventType:
        normalized.status === "arrived"
          ? "PARTNER_BOOKING_ARRIVED"
          : normalized.status === "completed"
            ? "PARTNER_BOOKING_COMPLETED"
            : normalized.status === "cancelled"
              ? "PARTNER_BOOKING_CANCELLED"
              : "PARTNER_BOOKING_UPDATED",
      sourceTable: "partner_bookings",
      sourceId: normalized.id,
      payload: { partnerBooking: normalized }
    });
...
async function syncDriverAttributionForReservation(supabase, reservation) {
  ...
  if (referral) {
    await enqueueIntegrationEventSafe(supabase, {
      ...
      eventType: ["qualified", "approved"].includes(referral.status)
        ? "DRIVER_REFERRAL_QUALIFIED"
        : "DRIVER_REFERRAL_UPDATED",
      sourceTable: "driver_referrals",
      sourceId: referral.id,
      payload: { referral, reservation, driver }
    });
  }
...
async function syncDriverAttributionForOrder(supabase, order) {
  ...
  const commission = await upsertDriverCommission(supabase, {
    branchId: order.branchId,
    driverId: driver.id,
  ...
  if (referral) {
    await enqueueIntegrationEventSafe(supabase, {
      branchId: referral.branchId,
      eventType: referral.status === "converted" ? "DRIVER_REFERRAL_CONVERTED" : "DRIVER_REFERRAL_UPDATED",
      sourceTable: "driver_referrals",
      sourceId: referral.id,
      payload: { referral, order, driver }
    });
  }
  ...
  if (commission) {
    await enqueueIntegrationEventSafe(supabase, {
      ...
      eventType: commission.status === "approved" || commission.status === "paid"
        ? "DRIVER_COMMISSION_APPROVED"
        : "DRIVER_COMMISSION_UPDATED",
      sourceTable: "driver_commission_transactions",
      sourceId: commission.id,
      payload: { commission, referral, order, driver }
    });
  }
...
export async function createReservation(supabase, payload) {
  ...
  await enqueueIntegrationEventSafe(supabase, {
    branchId: normalized.branchId,
    eventType: normalized.status === "confirmed" ? "BOOKING_CONFIRMED" : "BOOKING_CREATED",
    sourceTable: "reservations",
    sourceId: normalized.id,
    payload: { reservation: normalized }
  });
  return normalized;
}
...
export async function updateReservation(supabase, id, payload) {
  ...
  if (normalized) {
    await syncDriverAttributionForReservation(supabase, normalized);
    if (["confirmed", "arrived", "cancelled"].includes(normalized.status)) {
      await enqueueIntegrationEventSafe(supabase, {
        branchId: normalized.branchId,
        eventType:
...
export async function createVoucherLead(supabase, payload) {
  ...
  const normalized = normalizeVoucherRow(data);
  await enqueueIntegrationEventSafe(supabase, {
    branchId: normalized.branchId,
    eventType: "VOUCHER_ISSUED",
    sourceTable: "voucher_leads",
    sourceId: normalized.id,
    payload: { voucher: normalized }
  });
  return normalized;
}
...
export async function createPublicVoucherLead(supabase, payload) {
  ...
  const now = new Date().toISOString();
  const normalized = normalizeVoucherRow({
    id,
    ...values,
    created_at: now,
    updated_at: now
...
export async function redeemVoucherLead(supabase, voucherLeadId, payload = {}) {
  ...
  await enqueueIntegrationEventSafe(supabase, {
    branchId: updatedVoucher.branchId || target.branchId,
...
export async function listMediaAssets(supabase, options = {}) {
  const branchId = cleanText(options.branchId);
  const { data, error } = await runWithOptionalBranchFilter(branchId, (useBranch) => {
    let query = supabase
      .from("media_assets")
  ...
  throwIfError(error, "Không tải được media");
  return (data || []).map(normalizeMediaAssetRow);
}

export async function createMediaAsset(supabase, payload) {
  const values = buildMediaAssetValues(payload);
  if (!values.file_url) {
    throw new Error("Đường dẫn file không được để trống");
  }

  const { data, error } = await insertWithOptionalColumns(supabase, "media_assets", values, {
    select: true,
    single: true,
    optionalColumns: ["branch_id", "width", "height", "duration_seconds", "metadata"]
  });

  throwIfError(error, "Không tạo được media");
  return normalizeMediaAssetRow(data);
}

export async function updateMediaAsset(supabase, id, payload) {
  const values = buildMediaAssetValues(payload);
  if (!values.file_url) {
    throw new Error("Đường dẫn file không được để trống");
  }

  const { data, error } = await updateWithOptionalColumns(supabase, "media_assets", id, values, {
    optionalColumns: ["branch_id", "width", "height", "duration_seconds", "metadata"]
  });

  throwIfError(error, "Không cập nhật được media");
  return data ? normalizeMediaAssetRow(data) : null;
}

export async function deleteMediaAsset(supabase, id) {
  const { error } = await supabase.from("media_assets").delete().eq("id", id);
  throwIfError(error, "Không xóa được media");
  return true;
}
...
export async function listTableSessionEvents(supabase, options = {}) {
  const branchId = cleanText(options.branchId);
  const tableId = cleanText(options.tableId);
  const limit = Math.min(Math.max(parseInteger(options.limit, 120), 1), 500);
  let query = supabase
  ...
  if (error) {
    ...
    throwIfError(error, "Không tải được lịch sử bàn");
  }

  return (data || []).map(normalizeTableSessionEventRow);
}

export async function logTableSessionEvent(supabase, payload = {}) {
  const table = payload.table || {};
  const tableId = cleanText(payload.tableId || table.id);
  const toStatus = cleanText(payload.toStatus || table.status);
  if (!tableId || !toStatus) {
    return null;
  }
  ...
  if (latestError) {
    if (isSupabaseSchemaMissingError(latestError)) {
      return null;
    }
    throwIfError(latestError, "Không đọc được lịch sử bàn");
  }

  const startedAt = now.toISOString();
  const previousStartedAt = latest?.started_at ? new Date(latest.started_at).getTime() : NaN;
  const durationMinutes = Number.isFinite(previousStartedAt)
    ? Math.max(0, Math.round((now.getTime() - previousStartedAt) / 60000))
    : null;

  const values = {
    branch_id: cleanText(payload.branchId || table.branchId) || null,
    table_id: tableId,
    reservation_id: cleanText(payload.reservationId) || null,
    order_id: cleanText(payload.orderId) || null,
    event_type: cleanText(payload.eventType, "status_change") || "status_change",
    from_status: cleanText(payload.fromStatus) || null,
    to_status: toStatus,
    started_at: startedAt,
    duration_minutes: durationMinutes,
    actor_profile_id: cleanText(payload.actorProfileId) || null,
    actor_name: cleanText(payload.actorName),
    notes: cleanText(payload.notes),
    metadata: payload.metadata && typeof payload.metadata === "object" ? payload.metadata : {}
  };
  ...
  if (error) {
    ...
    throwIfError(error, "Không ghi được lịch sử bàn");
  }

  return data ? normalizeTableSessionEventRow(data) : null;
}
...
export async function createOrder(supabase, payload) {
  ...
  await enqueueIntegrationEventSafe(supabase, {
    branchId: normalized.branchId,
    eventType: normalized.status === "paid" ? "PAYMENT_SUCCESS" : "ORDER_CREATED",
    sourceTable: "orders",
    sourceId: normalized.id,
...
export async function updateOrder(supabase, id, payload) {
  ...
  const normalized = normalizeOrderRow(data);
  await syncDriverAttributionForOrder(supabase, normalized);
  if (["served", "paid", "cancelled"].includes(normalized.status)) {
    await enqueueIntegrationEventSafe(supabase, {
      branchId: normalized.branchId,
      eventType:
        normalized.status === "paid"
          ? "PAYMENT_SUCCESS"
          : normalized.status === "served"
            ? "ORDER_COMPLETED"
            : "ORDER_CANCELLED",
      sourceTable: "orders",
      sourceId: normalized.id,
...
export async function listIntegrationEvents(supabase, options = {}) {
  const branchId = cleanText(options.branchId);
  const { data, error } = await runWithOptionalBranchFilter(branchId, (useBranch) => {
    let query = supabase
  ...
  throwIfError(error, "Không tải được event đồng bộ");
  return (data || []).map(normalizeIntegrationEventRow);
}

export async function enqueueIntegrationEvent(supabase, payload = {}) {
  const eventType = cleanText(payload.eventType);
  const sourceTable = cleanText(payload.sourceTable);
  const providers = Array.isArray(payload.providers) && payload.providers.length
    ? payload.providers
    : ["uvfl", "goeco"];

  if (!eventType || !sourceTable) {
    return [];
  }

  const rows = providers.map((provider) => buildIntegrationEventValues(payload, provider));
  const { data, error } = await supabase
    .from("integration_events")
    .insert(rows)
    .select("*");
...
export async function updateIntegrationEventStatus(supabase, id, payload = {}) {
  const nextStatus = cleanText(payload.status, "pending") || "pending";
  const values = {
    status: nextStatus,
    last_error: nextStatus === "pending" ? "" : cleanText(payload.lastError),
    synced_at: nextStatus === "synced" ? new Date().toISOString() : toIsoDateTime(payload.syncedAt)
  };

  if (payload.retryCount !== undefined) {
    values.retry_count = parseInteger(payload.retryCount, 0);
  }

  const { data, error } = await updateWithOptionalColumns(supabase, "integration_events", id, values, {
    optionalColumns: ["synced_at"]
  });

  throwIfError(error, "Không cập nhật được event đồng bộ");
  return data ? normalizeIntegrationEventRow(data) : null;
}

function matchesIntegrationProvider(integration, provider) {
  ...
  return [
    ...
    .filter(Boolean)
    .some((value) => String(value).toLowerCase().includes(normalizedProvider));
}

function buildOutboundIntegrationEventPayload(event, integration) {
  return {
    eventId: event.id,
...
export async function syncIntegrationEventDb(supabase, id) {
  ...
  throwIfError(eventError, "Không tải được event đồng bộ");
  if (!eventRow) {
    return null;
  }

  const event = normalizeIntegrationEventRow(eventRow);
  const currentRetry = Number(event.retryCount || 0);
  await updateIntegrationEventStatus(supabase, id, {
    status: "processing",
    retryCount: currentRetry
  });

  const scopedIntegrations = await listIntegrationSettings(supabase, { branchId: event.branchId }).catch(() => []);
  const fallbackIntegrations = scopedIntegrations.length ? [] : await listIntegrationSettings(supabase).catch(() => []);
  const integration = [...scopedIntegrations, ...fallbackIntegrations].find(
    (item) => item.enabled && item.endpoint && matchesIntegrationProvider(item, event.provider)
  );

  if (!integration) {
    const updated = await updateIntegrationEventStatus(supabase, id, {
      status: "failed",
      retryCount: currentRetry + 1,
      lastError: `Chưa cấu hình endpoint đang bật cho provider ${event.provider}.`
    });
    throw new Error(updated?.lastError || "Chưa cấu hình endpoint cho provider");
  }

  const headers = {
    "Content-Type": "application/json",
    "x-sanhodo-event-id": event.id,
    "x-sanhodo-provider": event.provider
  };
  ...
  try {
    const response = await fetch(integration.endpoint, {
      method: "POST",
      headers,
      body: JSON.stringify(buildOutboundIntegrationEventPayload(event, integration))
    });
    ...
    if (!response.ok) {
      const updated = await updateIntegrationEventStatus(supabase, id, {
        status: "failed",
        retryCount: currentRetry + 1,
        lastError: `HTTP ${response.status}: ${responsePreview || response.statusText}`
      });
      throw new Error(updated?.lastError || `Đồng bộ thất bại: HTTP ${response.status}`);
    }
    ...
    const updated = await updateIntegrationEventStatus(supabase, id, {
      status: "failed",
      retryCount: currentRetry + 1,
      lastError: error.message || "Không gọi được endpoint đồng bộ"
    });
    throw new Error(updated?.lastError || "Không gọi được endpoint đồng bộ");
  }
}

export async function processIntegrationEventsBatch(supabase, options = {}) {
  const limit = Math.min(Math.max(parseInteger(options.limit, 10), 1), 25);
  const retryStatuses = Array.isArray(options.statuses) && options.statuses.length
    ? options.statuses
    : ["pending", "failed"];
  const maxRetry = Math.min(Math.max(parseInteger(options.maxRetry, 5), 1), 20);
  ...
  if (error && isSupabaseSchemaMissingError(error)) {
    return {
      ...
      failed: 0,
      skipped: true,
      reason: getSupabaseMigrationHint(error)
    };
  }

  throwIfError(error, "Không tải được event cần đồng bộ");

  const candidates = (data || []).map(normalizeIntegrationEventRow);
  const results = [];

  for (const event of candidates) {
    try {
      const synced = await syncIntegrationEventDb(supabase, event.id);
      results.push({
        id: event.id,
        ok: true,
        status: synced?.event?.status || "synced",
        provider: event.provider,
        eventType: event.eventType
      });
    } catch (error) {
...
async function enqueueIntegrationEventSafe(supabase, payload = {}) {
  try {
    return await enqueueIntegrationEvent(supabase, payload);
  } catch {
    return [];
  }
}
...
export async function getAdminDashboardData(supabase, options = {}) {
  ...
  const [
    ...
    listAdminLoginAudits(supabase, { branchId, limit: 160 }),
    ...
    listIntegrationEvents(supabase, { branchId, limit: 100 }),
    listMenuItems(supabase, { branchId }),
    listMediaAssets(supabase, { branchId }),
    listRestaurantTables(supabase, { branchId }),
    listTableSessionEvents(supabase, { branchId, limit: 160 }),
