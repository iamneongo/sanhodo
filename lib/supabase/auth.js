...
import { createAdminClient, createLocalAdminGuardedClient } from "./admin";
import { isSupabaseEnvMissingError } from "./config";
import { createClient } from "./server";

async function loadProfile(supabase, userId) {
  const { data, error } = await supabase
    .from("profiles")
    .select("id, email, full_name, role, branch_id, branch_code, is_active, last_login_at")
    .eq("id", userId)
    .maybeSingle();
...
export async function getCurrentSession() {
  ...
  if (isValidLocalAdminCookieValue(localAdminCookie)) {
    ...
    const localAdminSupabase = createAdminClient();
    return {
      supabase: localAdminSupabase || createLocalAdminGuardedClient(supabase),
      user: localAdmin.user,
      profile: localAdmin.profile,
      isLocalAdmin: true,
      localAdminWriteLimited: !localAdminSupabase
    };
  }
  ...
  if (error) {
    return { supabase, user: null, profile: null, isLocalAdmin: false, setupRequired: false, localAdminWriteLimited: false };
  }
  ...
  try {
    ...
    return { supabase, user, profile, isLocalAdmin: false, setupRequired: false, localAdminWriteLimited: false };
  } catch (profileError) {
