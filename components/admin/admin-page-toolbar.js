...
export default function AdminPageToolbar({ children, actions = null, footer = null, className = "" }) {
  return (
    <div
      className={cn(
        "grid gap-4 rounded-2xl border border-zinc-200 bg-zinc-50 p-4 md:grid-cols-[minmax(0,1fr)_auto] md:items-start",
        className
      )}
    >
      <div className="grid gap-3.5 sm:grid-cols-2 2xl:grid-cols-3">{children}</div>
